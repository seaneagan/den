...
main() {
  group('checkPackageName(String name)', () {
    test('may not be blank', () {
      expect(
        den.checkPackageName("").first.contains("may not be empty"),
        isTrue
      );
    });
    test('may only contain letters, numbers, and underscores', () {
      expect(
        den.checkPackageName(generateRandomName(useNonAlphaNumeric: true)).first
    ...
    test('must begin with a letter or underscore', () {
      expect(
        den.checkPackageName(generateRandomName(beginWithNumber: true)).first
          .contains("must begin with a letter or underscore"),
        isTrue
      );
    });
    test('may not be a reserved word in Dart', () {
      expect(
        den.checkPackageName(generateRandomName(useReservedWords: true)).first
          .contains("may not be a reserved word in Dart"),
        isTrue
      );
    });
    test('should be lower-case', () {
      expect(
        den.checkPackageName(generateRandomName(useUpperCase: true)).first
          .contains("should be lower-case"),
        isTrue
      );
    });
  });
  group('unCamelCase(String name)', () {
    test('', () {
      ...
      expect(
        nameCamel.split('_').every((token) => tokens.contains(token)),
        isTrue
      );
    });
  });
...
String generateRandomName(
  {
    bool useReservedWords: false,
    bool beginWithNumber: false,
    bool useNonAlphaNumeric: false,
